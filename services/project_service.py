...
class Project(BaseClient):
    def create_project(self, create_project_payload):
        ...
        return requests.post(url, json.dumps(create_project_payload), headers=headers)
    ...
    def upload_project_file(self, project_id, upload_project_file_payload):
        ...
        headers = self.headers_with_token()
        return requests.post(url, json.dumps(upload_project_file_payload), headers=headers)
